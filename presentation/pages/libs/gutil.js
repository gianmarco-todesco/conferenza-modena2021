...
class VertexDataBuilder {
    ...
    addTriangle(p0, p1, p2, y) {
        let nrm = BABYLON.Vector3.Cross(p1.subtract(p0),p2.subtract(p0)).normalize();
        const positions = []
        const normals = []
        const indices = []
        const uvs = []
        const offset = nrm.scale(y);
        if(y<0) nrm = nrm.scale(-1);
        [p0,p1,p2].forEach(p=>{
        ...
        if(y>=0) indices.push(0,2,1);
        else indices.push(0,1,2);
        const vd = new BABYLON.VertexData()
        vd.positions = positions
        vd.normals = normals
        vd.indices = indices
