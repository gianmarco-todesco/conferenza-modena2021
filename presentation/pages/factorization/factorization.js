...
function Q() {
    ...
    // console.log("ecco",count++);
    // slideTick();
    if(running) window.requestAnimationFrame(Q)
}
...
window.addEventListener('DOMContentLoaded', function() {
    ...
    if (!canvas.getContext || !canvas.getContext("2d")) {
        ...
        // defineSlide('fact', startSlide, stopSlide);
    }
});
var count = 0;
...
*/
function setup() {
    if(!running) { running = true; window.requestAnimationFrame(Q); }
...
window.addEventListener("message", function(e) {
    if(e.data == "slide:stop") { stopSlide(); }
    else if(e.data == "slide:start") { startSlide(); }
}, false);
*/